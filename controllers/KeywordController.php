...
class KeywordController extends Controller
{
    ...
    public function actionTransform($id){
        $model = $this->findModel($id);
        $count = HotWord::find()->where(['hot_word' => $model->hot_word])->count();
        if($count > 0){
            Yii::$app->getSession()->setFlash('warning', Yii::t('app', 'keyword Exist'));
        }else{
            $hotWord = new HotWord();
            $hotWord->hot_word = $model->hot_word;
            $hotWord->number = HotWord::find()->count() + 1;
            $hotWord->save();
            Yii::$app->getSession()->setFlash('success', Yii::t('app', 'save_succeeded'));
        }
        return $this->redirect(['index']);
